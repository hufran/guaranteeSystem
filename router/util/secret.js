...
const encodeString=(userId)=>{
  let shaString=userId+';'+configure.prod.frontSecret;

  const cipher = crypto.createCipher('aes192', configure.prod.frontSecret);
  var crypted = cipher.update(shaString, 'utf8', 'hex');
  crypted += cipher.final('hex');
  return crypted;
};

const decodeString=(crypted)=>{
  const decipher = crypto.createDecipher('aes192', configure.prod.frontSecret);
  let decrypted = decipher.update(crypted, 'hex', 'utf8');
  decrypted += decipher.final('utf8');
  let userPass=decrypted.split(';')[0];
  return userPass;
};
...
module.exports={createAccessToken,decodeString,decodeAccessToken};
